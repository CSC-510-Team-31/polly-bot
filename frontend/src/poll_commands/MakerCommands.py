...
class PollButtons(discord.ui.View):
    ...
    @discord.ui.button(label="List Questions", row=1, style=discord.ButtonStyle.primary)
...
class MCPollModal(discord.ui.Modal):
    ...
    async def callback(self, interaction: discord.Interaction):
        self.val = MCQuestion(self.children[0].value, self.children[0].value)
...
class MakerCommands(commands.Cog):
    ...
    async def make_poll(self, ctx: ctx_parse, poll_name: str):
        ...
        print(poll_name)
        await ctx.send(f'Buttons for poll {poll_name}: ', view=PollButtons(poll_name=poll_name, question_dict=self.question_dict))
